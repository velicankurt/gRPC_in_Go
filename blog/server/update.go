...
func (s *Server) UpdateBlog(ctx context.Context, in *pb.Blog) (*emptypb.Empty, error) {
	log.Println("UpdateBlog function was invoked with: ", in)
	...
	res, err := collection.UpdateOne(ctx,
		...
		bson.M{"$set": data})
	if err != nil {
		return nil, status.Errorf(codes.Internal, fmt.Sprintf("failed to update data: %v", err))
	}

	if res.MatchedCount == 0 {
		return nil, status.Errorf(codes.NotFound, fmt.Sprintf("no blog found with id: %s", in.Id))
	}
